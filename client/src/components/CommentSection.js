            ...
            <div className='mt-8 '>
                {blogComments && blogComments.map((comment, index) => {
                    return (
                        ...
                        <div key={index} className="bg-gray-100 rounded-md py-2 px-4 lg:px-8 flex">
                            ...
                            <div className="flex flex-col items-start mt-2  mx-0">
                                <a href={`/profile/${comment?.userId?._id}/${comment?.userId?.name}`} className="text-black text-xs ">{comment?.username}</a>

                              <p className="text-black text-sm flex  font-semibold  ">{comment?.commentDesc}</p>

                            {(userInfo?._id === comment?.userId?._id)? (<button onClick={() => deleteComment(comment?._id)} className='text-xs  text-black mt-2 '>Delete</button>) : ('')}
                            </div>
