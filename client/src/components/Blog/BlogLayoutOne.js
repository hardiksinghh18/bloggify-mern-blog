...
const BlogLayoutOne = ({ blog }) => {
    ...
    return (
        <div className="group flex flex-col w-[280px] sm:w-[340px] md:w-[380px] bg-white dark:bg-[#1e1e1e] rounded-[20px] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-800">
            <Link to={`/blogs/${blog?.slug}`} className="relative h-[180px] sm:h-[200px] md:h-[220px] w-full overflow-hidden block">
                <img 
            ...
            <div className="p-5 sm:p-6 flex flex-col flex-grow">
                <div className="flex items-center gap-3 mb-3 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                    <span className="font-semibold text-[#b8004e] dark:text-[#d81b60] bg-[#b8004e]/10 dark:bg-[#d81b60]/20 px-2 py-1 rounded-md text-[11px] uppercase tracking-wide">
                        Article
                    </span>
                    <span>{formattedDate}</span>
                </div>
                
                <Link to={`/blogs/${blog?.slug}`} className="block mb-4">
                    <h2 className="font-bold text-gray-900 dark:text-white text-lg sm:text-xl line-clamp-2 leading-tight group-hover:text-[#b8004e] dark:group-hover:text-[#d81b60] transition-colors duration-300">
                        {blog?.title}
                    </h2>
                </Link>
                ...
                <div className="mt-auto flex items-center gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex flex-col">
                        <Link to={`/profile/${blog?.author?.username}`} className="text-sm font-semibold text-gray-900 dark:text-gray-100 hover:underline">
                            {blog?.author?.name}
                        </Link>
